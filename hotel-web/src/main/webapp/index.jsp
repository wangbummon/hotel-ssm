                                ...
                                <c:if test="${room.floorId == floor.id }">
                                    <div data-id="${room.id}" class="layui-col-xs6 layui-col-md3">
                                        <a class="template store-list-box fly-case-active"
                                           href="/room/detail/${room.id}" data-type="toRoomInfo">
                                            <img src="/hotel/show/room/main/${room.photo}"
                                                 class="store-list-cover">
                                            <h2 class="layui-elip">${room.title}</h2>
                                            <p class="price"><span title="金额"
                                                                   style="color: red;font-size: 16px;padding: 3px;"> ￥${room.roomType.price} </span>
                                                <span title="房号"
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        html += "<dd data-id='" + data[i].id + "'>";
                        html += "<a class='fly-case-active' href='/room/list/" + data[i].id + ".html' data-type='toRoomTypeList'>" + data[i].typeName + "</a>";
                        html += "</dd>";
                    }
                    // 将网页追加到dl标签中
